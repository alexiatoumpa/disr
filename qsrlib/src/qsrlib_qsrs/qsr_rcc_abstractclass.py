...
class QSR_RCC_Abstractclass(QSR_Dyadic_1t_Abstractclass):
    ...
    def _compute_qsr(self, bb1, bb2, qsr_params, **kwargs):
        ...
        '''
        If the overlapping area is smaller than 10% over the smallest object area
        then it is not considered as a "po" relation.
        '''
        #'''
        areaA = (abs(ax-bx) * abs(ay-by))
        areaB = (abs(cx-dx) * abs(cy-dy))
        if areaA <= areaB:
            minArea = areaA
        else:
            minArea = areaB

        overlapArea = (abs(min(ay, cy) - max(by, dy)) * abs(max(ay, cy) - min(by, dy)))
        if overlapArea <= (0.1 * minArea):
            return self._convert_to_requested_rcc_type("ec")
        else:
            return self._convert_to_requested_rcc_type("po")
